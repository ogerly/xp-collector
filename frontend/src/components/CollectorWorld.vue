    ...
    <b-container class="bv-example-row border mb-4 mt-4 p-4">
      <b-row>
        <b-col class="border2">
          ...
          <hr>
          <ul>
            <li v-for="item in nodeItems" :key="item.id"><div @click="editNode">{{item.Tables_in_demo_db}}</div></li>
          </ul>
        </b-col>
        <b-col class="border3">
        ...
        <b-col >
          <div show v-show="nodeContentText != ''" >
            ...
            >
              ...
              <b-input-group-append>
                <b-button size="sm" text="Button" variant="success" @click="saveNodeContent">save</b-button>
            ...
            <hr>
             <ul>
            <li v-for="item in nodeContentItems" :key="item.id"><div @click="editNodeContent">{{item.node_text}}</div></li>
          </ul>
  ...
  methods: {
    ...
    async getNodes () {
      try {
        ...
        this.nodeItems = response.data
        console.log(response.data)
        this.optionsNode1 = response.data
      } catch (err) {
        console.log(err)
      }
    },
    ...
    async saveNode () {
      ...
      try {
        await axios.post('http://localhost:5000/node', {
